...
# PART 12 — ANTIGRAVITY EXECUTION PROMPT

## Pre-Read Instructions for the Human Operator

Before handing this prompt to Antigravity:
1. Ensure the entire repository structure has been initialized with `git init`
2. Have Docker running
3. Have the SENTINELS v2.0 Master Blueprint document open as context
4. The entire codebase context should be empty (new repo) — Antigravity will build from scratch following this document
...
```
ROLE: You are Antigravity, a senior full-stack engineer and DevOps architect executing a 
complete software project build. You write production-quality, fully-connected, actually-working 
code — not placeholder stubs, not "// TODO implement this", not dummy data unless explicitly 
specified. Every piece of code you write must be real, runnable, and tested.

═══════════════════════════════════════════════════════════════════════════════
PROJECT: SENTINELS v2.0 — AI-Driven Self-Healing Cloud Deployment System
═══════════════════════════════════════════════════════════════════════════════

YOUR FIRST THREE ACTIONS (mandatory before writing a single line of code):

1. ANALYZE THE CODEBASE
   Use your file system tools to scan the entire existing repository structure.
   Understand every existing file, its purpose, and its current state.
   Run: list_files("/"), read all existing package.json, requirements.txt, 
   docker-compose.yml files. Understand what has already been built.
   
2. READ THE MASTER BLUEPRINT
   Carefully read the attached SENTINELS_v2_Master_Blueprint document (all parts).
   The document is your specification — every architectural decision, port assignment,
   data flow, and algorithm choice is specified there. Do not deviate from specifications
   unless you identify a genuine technical impossibility and explain why.

3. CREATE YOUR EXECUTION PLAN
   Before writing any code, create a file called EXECUTION_PLAN.md that lists:
   - Every file you will create or modify
   - The order in which you will build them (dependencies first)
   - Specific test you will run after each component to verify it works
   - Estimated token count for the full build
   
   Structure your plan as tasks, not phases. E.g.:
   Task 1: Create sentinels/healer-agent/main.py (FastAPI app, all endpoints)
   Task 2: Create sentinels/healer-agent/ml/anomaly_detector.py (Isolation Forest)
   ... and so on
...
Three interconnected applications, all running locally on a laptop via k3d Kubernetes:

APPLICATION 1: NetflixOS (Port 3001)
- Microservices architecture: 8 FastAPI backend services + 1 React frontend
- Looks and functions like Netflix (dark red theme, card browsing, video playback)
- Movies use YouTube IFrame API for video playback (no real video hosting needed)
- 50 pre-seeded movies with real YouTube video IDs (trailers)
- Full authentication (JWT), search, recommendations, fake payments
- CRITICAL: Each service independently deployable, independently killable
- When Search Service dies, only search breaks — everything else continues

APPLICATION 2: PrimeOS (Port 3002)  
- Monolithic architecture: Single Django 5.0 backend + 1 React frontend
- Looks and functions like Amazon Prime Video (navy/gold theme)
- Same YouTube IFrame approach for video playback
- CRITICAL: When the single Django pod dies, THE ENTIRE SITE shows outage screen
- This architectural contrast vs Netflix is the core demo value

APPLICATION 3: SENTINELS Command Center (Port 3000)
- React + TypeScript frontend with dark "mission control" aesthetic
- MAIN FEATURE: 3D force-directed graph (React Three Fiber / Three.js)
  * Shows every Kubernetes pod as a colored 3D sphere
  * Colors: green=healthy, amber=warning, red=critical, blue=healing
  * When attack launched: red particle streams animate toward target node
  * When healing: blue particles stream from SENTINELS core node to target
  * Full OrbitControls (drag to rotate, scroll to zoom)
- Attack Launcher: dropdown to select target + attack type + intensity + duration
- Healing Log: live feed of all healing events (Socket.IO real-time)
- Metrics Scorecard: F1, MTTD, MTTR, FPR, Recovery Rate (calculated, not fake)
- Grafana embedded: iframe showing Netflix RED dashboard

SENTINELS CORE ENGINE (runs in sentinels-system namespace):
- Healer Agent (FastAPI :5000): receives AlertManager webhooks, full 8-step pipeline
- ML Pipeline: Isolation Forest anomaly detection (scikit-learn)
- Policy Engine: OPA/Rego decisions (OPA server :8181)
- Kubernetes Operator: Kopf-based healing execution
- Metrics Aggregator (FastAPI :5050): calculates academic metrics
- Safety Stack: circuit breaker + cooldown + blast radius in Redis

MONITORING (monitoring namespace):
- kube-prometheus-stack: Prometheus + AlertManager + Grafana + kube-state-metrics
- Loki: log aggregation
- Custom PrometheusRule alerts: HighCPUUsage, PodCrashLooping, HighMemoryUsage, etc.

CHAOS ENGINEERING (chaos namespace):  
- Chaos Mesh v2.7.3: infrastructure fault injection
- k6: HTTP flood simulation
- 10 attack types all wired to the dashboard Attack Launcher

═══════════════════════════════════════════════════════════════════════════════
CODING STANDARDS — NON-NEGOTIABLE
═══════════════════════════════════════════════════════════════════════════════

1. GET SHIT DONE MENTALITY
   - Write complete, working implementations. Not partial.
   - If you start a file, finish it completely before moving on.
   - Never leave TODO comments for core functionality. Implement it.
   - Exception: only leave TODOs for genuinely optional/future features

2. SEQUENTIAL THINKING AND PLANNING
   Before implementing any component:
   a) State what you are about to build (1 sentence)
   b) List the inputs and outputs of this component
   c) List all dependencies this component needs (must exist before this)
   d) Identify 3 potential failure modes and how you're preventing them
   THEN write the code.

3. REAL CONNECTIONS — NO MOCKS IN PRODUCTION CODE
   - Every API call must use real URLs (Kubernetes service DNS names)
   - Every database query must use real SQL/ORM (no in-memory substitutes)
   - Every Socket.IO event must be real events with real data
   - Exception: YouTube videos (use real YouTube IDs, just not original Netflix content)
   - Exception: Payment processing (simulate, clearly labeled as simulation)

4. TEST YOUR OWN WORK
   After creating each major component, create a test script and verify:
   - FastAPI services: test all endpoints with httpx/requests
   - React components: verify they render without errors (no browser tests needed)
   - Kubernetes manifests: validate with `kubectl apply --dry-run=client`
   - Helm charts: validate with `helm lint`
   - Python ML: verify model trains and produces sensible output on test data
   
   If a test fails, FIX IT before moving to the next task. Do not carry forward bugs.

5. DOCKER BUILDS MUST WORK
   Every Dockerfile must:
   - Build successfully (no missing dependencies)
   - Run successfully (service starts and responds to /health)
   - Be optimized: multi-stage builds for production, minimal base images
   - Use non-root user (runAsNonRoot: true)

6. ERROR HANDLING EVERYWHERE
   - Every async function has try/except with meaningful error messages
   - Every HTTP endpoint returns proper status codes (400 for bad input, 503 for dependency failure, etc.)
   - Every Kubernetes API call handles NotFound, Forbidden, and ApiException
   - Redis connection failures fall back gracefully (circuit breaker defaults to CLOSED if Redis unavailable)

7. PROMETHEUS METRICS ON EVERY SERVICE
   Every FastAPI service exposes /metrics endpoint with at minimum:
   - http_requests_total{method, endpoint, status_code, service}
   - http_request_duration_seconds{method, endpoint, service}
   Use prometheus-client library, mount ASGI app at /metrics

8. STRUCTURED LOGGING
   All backend services use Python structlog or standard logging with JSON format:
   {"timestamp": "...", "level": "INFO", "service": "search-service", 
...
PORT ASSIGNMENTS (do not change these):
- SENTINELS Dashboard: 3000
- Netflix Frontend: 3001  
- Prime Frontend: 3002
- Grafana: 3003
- Healer Agent: 5000
- Metrics Aggregator: 5050
- Netflix API Gateway: 8001
- Netflix User Service: 8002
- Netflix Content Service: 8003
- Netflix Streaming Service: 8004
- Netflix Search Service: 8005
- Netflix Recommendation Service: 8006
- Netflix Payment Service: 8007
- Netflix Notification Service: 8008
- PrimeOS Backend: 8020
- Prometheus: 9090
- AlertManager: 9093
- OPA Server: 8181

KUBERNETES NAMESPACES:
- netflix (for all Netflix services)
- prime (for PrimeOS)
- monitoring (for Prometheus stack)
- sentinels-system (for healer, OPA, Redis, PostgreSQL)
- chaos (for Chaos Mesh)
- dashboard (for Command Center)

ISOLATION FOREST PARAMETERS (from research — do not change):
- n_estimators: 200
- contamination: 0.02
- max_samples: 256
- random_state: 42
- Anomaly threshold: -0.10 (any score below = anomaly)
- CRITICAL severity: score <= -0.35
- WARNING severity: score <= -0.20

ALERTMANAGER → HEALER:
- Webhook URL: http://healer-agent.sentinels-system.svc.cluster.local:5000/alerts
- AlertManager config group_wait: 10s (reduced for demo speed)
- group_interval: 30s

3D GRAPH COLORS:
- Healthy: #00FF88 (neon green)
- Warning: #FFD700 (gold)
- Degraded: #FF6600 (orange)
- Critical: #FF0000 (red)
- Healing: #0088FF (electric blue)
- Recovered: #00FF88 (green, after transition)
- SENTINELS core: #00CCCC (teal, animated)

YOUTUBE INTEGRATION:
- Use youtube-nocookie.com for embeds (no tracking)
- Player: `https://www.youtube-nocookie.com/embed/{VIDEO_ID}?autoplay=1`
- Pre-seed 50 movies with valid YouTube IDs for trailers
  (Use movie trailers that are publicly available — BigBuckBunny for testing)

═══════════════════════════════════════════════════════════════════════════════
BUILD ORDER (follow this exactly — respect dependencies)
═══════════════════════════════════════════════════════════════════════════════

PHASE A: Infrastructure Foundation
1. Repository structure and Makefile
2. Docker Compose for local development (docker-compose.dev.yml)
3. PostgreSQL initialization scripts (schemas for netflix_users, netflix_content, 
   healing_audit_log, prime_users, prime_content)
4. Redis configuration
5. k3d cluster creation script and namespace manifests
6. RBAC configurations

PHASE B: Netflix Microservices (build in this order - gateway last)
7. netflix/user-service (FastAPI, PostgreSQL, JWT)
8. netflix/content-service (FastAPI, PostgreSQL, 50 seeded movies with YouTube IDs)
9. netflix/search-service (FastAPI, SQLite FTS5)
10. netflix/streaming-service (FastAPI, YouTube delegation, Redis tracking)
11. netflix/recommendation-service (FastAPI, pre-computed recommendations in Redis)
12. netflix/payment-service (FastAPI, simulated payments)
13. netflix/notification-service (FastAPI, simulated notifications)
14. netflix/api-gateway (FastAPI, routes to all above, JWT validation)
15. netflix/frontend (React, Netflix design, all pages functional)

PHASE C: PrimeOS Monolith
16. prime/backend (Django 5.0, all 6 modules, SQLite, django-prometheus)
17. prime/frontend (React, Prime Video design)

PHASE D: Kubernetes Manifests
18. Dockerfiles for all services (build and test each one)
19. Kubernetes Deployment/Service YAMLs for all services
20. Helm chart structures (netflix/, prime/, sentinels/, dashboard/ charts)
21. kube-prometheus-stack values.yaml
22. PrometheusRule CRDs (all alert definitions)
23. PodDisruptionBudgets for Netflix services

PHASE E: SENTINELS Core Engine  
24. sentinels/healer-agent/ml/feature_engineering.py
25. sentinels/healer-agent/ml/anomaly_detector.py
26. sentinels/healer-agent/ml/prometheus_collector.py
27. sentinels/opa-policies/healing_policy.rego
28. sentinels/opa-policies/safety_policy.rego
29. sentinels/healer-agent/safety/circuit_breaker.py
30. sentinels/healer-agent/safety/cooldown_manager.py
31. sentinels/healer-agent/models/database.py (SQLAlchemy models + async engine)
32. sentinels/healer-agent/policy/opa_client.py
33. sentinels/healer-agent/main.py (COMPLETE healer with all 8 steps)
34. sentinels/metrics-aggregator/main.py (F1, MTTD, MTTR calculations)

PHASE F: Dashboard
35. dashboard/backend/main.py (topology API, attack launcher, Kubernetes client)
36. dashboard/frontend/src/hooks/useWebSocket.ts
37. dashboard/frontend/src/hooks/useTopology.ts
38. dashboard/frontend/src/components/TopologyGraph3D/ (Three.js 3D graph)
39. dashboard/frontend/src/components/AttackLauncher/
40. dashboard/frontend/src/components/HealingLog/
41. dashboard/frontend/src/components/MetricsScorecard/
42. dashboard/frontend/src/components/GrafanaEmbed/
43. dashboard/frontend/src/App.tsx (layout, all panels together)

PHASE G: Chaos Engineering
44. chaos/experiments/ (all 10 Chaos Mesh YAML templates)
45. chaos/k6-scripts/http-flood.js
46. Chaos Mesh Helm values

PHASE H: Final Integration
47. Grafana dashboard JSONs (3 dashboards)
48. Loki configuration
49. scripts/setup.sh (one-command demo setup)
50. scripts/run-evaluation-suite.sh (30-trial evaluation)
51. README.md (complete setup instructions)
52. Integration tests: launch full system, run one complete attack → heal cycle

═══════════════════════════════════════════════════════════════════════════════
SELF-VERIFICATION REQUIREMENTS
═══════════════════════════════════════════════════════════════════════════════

After completing EACH phase, you MUST verify:

After Phase B (Netflix):
□ All 8 services start without errors
□ GET /health on all services returns 200
□ GET /metrics on all services shows prometheus metrics
□ Netflix frontend loads at localhost:3001
□ Can login, browse movies, click play → YouTube video appears

After Phase C (Prime):
□ Django server starts without errors  
□ Prime frontend loads at localhost:3002
□ Stop Django → entire Prime site shows outage screen

After Phase D (Kubernetes):
□ All Docker images build: docker build succeeds for every service
□ helm lint passes for all charts
□ kubectl apply --dry-run=client succeeds for all manifests
□ k3d deployment: kubectl get pods shows all services Running

After Phase E (SENTINELS):
□ Isolation Forest trains on test data without errors
□ OPA evaluates test policy: `opa eval -d healing_policy.rego 'data.sentinels.healing.action'`
□ Healer starts without errors, /health returns 200
□ Manual AlertManager webhook POST to /alerts → healer logs show processing

After Phase F (Dashboard):
□ Dashboard frontend compiles without TypeScript errors
□ 3D graph renders (no WebGL errors in console)
□ Attack launcher sends request to backend API
□ Healing log receives events via Socket.IO

After Phase H (Full Integration):
□ Run demo-setup.sh → all services start successfully
□ Open dashboard → 3D graph shows all Netflix and Prime pods
□ Launch CPU stress attack on Netflix Search → 
  3D graph shows attack animation → 
  SENTINELS detects within 90s → 
  Pod restarts → 
  Graph shows healing animation → 
  Healing log shows SUCCESS entry →
  Metrics update showing MTTD and MTTR
□ Kill PrimeOS pod → entire Prime site goes down → SENTINELS heals it
□ Emergency stop button works: removes all active attacks immediately

═══════════════════════════════════════════════════════════════════════════════
WHAT THIS IS FOR
═══════════════════════════════════════════════════════════════════════════════

This is a university capstone project for a BE student (Ayush) building an 
AI-driven self-healing Kubernetes platform. The evaluators are professors who 
will be watching a LIVE DEMO. The software must:

1. Look professional and polished — UI design matters
2. Work reliably under demo conditions — no crashes, no missing data
3. Show clear cause-and-effect — launch attack → see it on 3D graph → 
   SENTINELS detects → heals → metrics update
4. Be explainable — every healing decision has a reason that can be shown

The target is IMPRESSIVE the professors who have underestimated Ayush's abilities.
Build this with the craft and quality that proves their assessment wrong.

═══════════════════════════════════════════════════════════════════════════════
STITCHING / DESIGN MCP
═══════════════════════════════════════════════════════════════════════════════

For all UI design work (React frontends), use the STITCH MCP tool to:
1. Generate high-quality component designs
2. Ensure the Netflix frontend matches Netflix's actual design language
3. Ensure the Prime frontend matches Amazon Prime Video's design language  
4. Ensure the SENTINELS dashboard matches a professional "mission control" aesthetic

Design priorities:
- Netflix: Pure black (#000000), red (#E50914), white text, Bebas Neue titles
- Prime: Deep navy (#0F171E), Prime blue (#00A8E1), Amazon yellow (#F5E72E)
- SENTINELS: Near-black (#050A14), electric cyan (#00D4FF), neon green (#00FF88)

═══════════════════════════════════════════════════════════════════════════════
BEGIN
═══════════════════════════════════════════════════════════════════════════════

Start now. First action: analyze any existing codebase files.
Second action: read the Master Blueprint document completely.
Third action: write EXECUTION_PLAN.md with your complete task list.
Then begin Task 1.

Do not ask for permission. Do not ask clarifying questions unless you encounter
a genuine impossibility. Make intelligent decisions based on the blueprint.
When in doubt, refer to the blueprint. When the blueprint is silent, use
your best engineering judgment.

Build something that makes Ayush proud.
```
